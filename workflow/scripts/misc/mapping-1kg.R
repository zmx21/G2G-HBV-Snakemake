...
FILE_REF_RAW <- "/home/mxu/G2G_TB/1000_Genomes/joined.1000genomes"
system(glue::glue("mkdir -p {DIR_SCRATCH}/mapping-1kg/"))
...
tmp <- readr::read_delim(glue::glue("{FILE_TARGET}.bim"),
             delim = "\t",
             col_names = FALSE)

target_snps <- data.frame(tmp$X2)
readr::write_delim(target_snps, glue::glue("{DIR_PROCESSED}/target_snps.txt"),
             delim = " ",
             col_names = FALSE)
...
idlist_pop_REF <- data.table::fread("/home/naret/work/data/1KG/other_data/integrated_call_samples_v3.20130502.ALL.panel") %>% dplyr::select(sample, pop, super_pop)
...
ggplot2::ggplot() + 
  ...
  geom_point(data = pc_TARGET, aes(PC3, PC4, shape = hbv), alpha = I(0.8)) + 
  scale_shape_manual(values = c(1, 16))


ggplot2::ggplot() + 
  geom_point(data = pc_REF, aes(PC5, PC6, color = `1KG`)) + 
  geom_point(data = pc_TARGET, aes(PC5, PC6, shape = hbv), alpha = I(0.8)) + 
  scale_shape_manual(values = c(1, 16))
